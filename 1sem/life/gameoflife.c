...
int main(int argc, char *argv[]){
  ...
  int max_iter = 1;        // infinity
  int dump_freq = 1;


  for (i = 1; i < argc; i++) {    // reading arguments
    ...
    for (iter = 1; iter < max_iter+1; iter++){
      ...
      for (i = 0; i < my; i++) {      // creating neighbours matrix
        for (j = 0; j < mx; j++) {
          ...
          if (firstgen[ i*mx + ((j-1 + mx) % mx)] == 0) currNeighbours++;
          if (firstgen[ i*mx + ((j+1 + mx) % mx)] == 0) currNeighbours++;
      ...
      if (iter % dump_freq == 0) {
        ...
        strcat(tempFile, "out");
        strcat(tempFile, c);
        saveBMP(strcat(tempFile, ".bmp"), firstgen, mx, my);
        memset(tempFile, 0, 20);
      }
    }
  } else printf("ERROR\n" );
